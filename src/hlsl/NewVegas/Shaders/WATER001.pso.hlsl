...
PS_OUTPUT main(PS_INPUT IN, float2 PixelPos : VPOS) {
    ...
    if (TESR_WaterLighting2.w > 0.5f)
        OUT.color_0 = float4(waterDebugView(TESR_WaterLighting2.w, shadow, transmittance, fresnel, scattering, specRoughness, pointLights, waterPath, foam, 1.0f, caustics), 1.0f);
